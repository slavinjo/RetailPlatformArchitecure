...
- **Monolit** — jedna aplikacija/baza, sve sinkrono, ACID besplatno. Odbačeno: onemogućuje nezavisnu isporuku po timu i nezavisno skaliranje vrućih putanja; na ovom volumenu postaje usko grlo. *("Monolith first" je legitiman za jedan tim / neizvjesnu domenu — ovdje ne vrijedi.)*
...
**Odluka.** **Event-driven arhitektura, ali hibridno** — sinkrono (contract-first REST) gdje je prirodno zahtjev-odgovor (npr. Cart → Catalog get-by-id), a asinkroni domenski eventi (RabbitMQ) za međudomenske tokove gdje decoupling nosi vrijednost (Order → Payment → Inventory, marketplace sync). Bez event sourcinga kao defaulta.
...
- *Negativno (i kako se adresira):* eventual consistency → saga s kompenzacijama (Sekcija 3.7); dual-write → outbox (3.6); teže debugiranje → korelacijski ID + distribuirani tracing (3.7, 8); pad poruka → retry + DLQ (3.6).
...
- **Fino zrnati mikroservisi (20+)** — maksimalna nezavisnost. Odbačeno: dva tima ne mogu operativno održavati toliko servisa; prerana fragmentacija → distribuirana složenost bez koristi.
...
**Posljedice.** *Pozitivno:* nezavisna isporuka i skaliranje po domeni; granice = vlasništvo timova (Conway). *Negativno:* distribuirana složenost (adresirana u 3.6/3.7). *Granica:* daljnja podjela servisa samo uz konkretan pokretač (skaliranje/autonomija).
...
**Razmotrene alternative.**
- **Jedna dijeljena baza za sve servise** — jednostavnija operativno. Odbačeno: skriveno spajanje kroz shemu → "distribuirani monolit", gubi se nezavisna deployabilnost.
...
**Posljedice.** *Pozitivno:* jednostavnost, nizak teret, pokriva real-time pub/sub. *Negativno:* nije log → nema besplatnog povijesnog replaya; rješava se outboxom + DLQ replayem. *Okidač za promjenu:* tvrd zahtjev za arbitrarnim povijesnim replayem / event sourcingom → uvođenje Kafke.
...
**Posljedice.** *Pozitivno:* jeftino, skalabilno, CDN-friendly; baza ostaje vitka. *Negativno:* dva sustava za održavati (store + metapodaci). *Sigurnost:* računi su immutable (korekcija = nova storno faktura), write-once uz retention zbog zakonske obveze čuvanja.

## ADR-007: Observability backend — self-hosted, managed kao opcija

**Status:** Prihvaćeno

**Kontekst.** Treba odabrati gdje se telemetrija sprema, vizualizira i alarmira. Instrumentacija je već standardizirana na OpenTelemetry (Sekcija 8.1), pa je backend odvojiv. Sustav je cloud-agnostičan i izbjegava vendor lock-in (Sekcija 3.4), ali observabilnost na razini milijuna korisnika nosi realan operativni teret.
...
**Odluka.** **Self-hosted Prometheus/Grafana kao default** — dosljedno s cloud-agnostičnom, lock-in-averznom linijom. Budući da svi servisi šalju **OpenTelemetry**, backend je **zamjenjiv**: managed platforma (New Relic/Datadog/Grafana Cloud) je drop-in alternativa ako operativni teret nadmaši vrijednost.
